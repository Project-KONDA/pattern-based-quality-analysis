...
public class FormulaFilterPartImpl extends BooleanFilterPartImpl implements FormulaFilterPart {
	...
	public FormulaFilterPartImpl(ObjectNode json, Map<Integer, InterimResultPart> map) throws InvalidityException {
		super();
		try {
			setOperator(LogicalOperator.get(json.get("operator").asText()));
			FixedContainerInterimImpl argument = (FixedContainerInterimImpl) map.get(json.get("argument").asInt());
			setArgument(argument);
			setSubfilter1((BooleanFilterPart) JavaFilterPartImpl.fromJson((ObjectNode) json.get("subfilter1"), map));
			setSubfilter2((BooleanFilterPart) JavaFilterPartImpl.fromJson((ObjectNode) json.get("subfilter2"), map));
		}
		catch (Exception e) {
